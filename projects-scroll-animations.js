...
export function initProjectsScrollAnimations() {
  ...
  panels.forEach((panel, index) => {
    ...
    // Animate elements in sequence (optimized timing with GPU acceleration)
    if (projectInfo) {
      tl.to(projectInfo, { 
        opacity: 1, 
        duration: 0.4, 
    ...
    // Project card animation (optimized for performance)
    if (projectCard) {
      if (!isMobile) {
        tl.to(projectCard, {
          opacity: 1,
    ...
    // Optimized parallax (only on desktop, single ScrollTrigger per panel)
    if (projectCard && !isMobile) {
      gsap.to(projectCard, {
        y: isOdd ? -20 : 20,
        ease: 'none',
        force3D: true, // GPU acceleration for smooth parallax
        scrollTrigger: {
          trigger: panel,
          start: 'top bottom',
          end: 'bottom top',
          scrub: 1.5, // Smoother scrub value for better performance
        }
      });
    }
  });
